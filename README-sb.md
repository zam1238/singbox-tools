...
## ② argo（Cloudflare Argo 开关）,当前只能vmess或者trojan 这2者选1个来弄argo，暂时不支持同时argo。
...
## ④ cdn_host、cdn_pt、hy_sni、vl_sni、vl_sni_pt（cdn域名 和 各协议的伪装域名，可选)
...
## ⑦ reality_private  此为vless Reallity协议节点的密钥。
...
## ⑩ argo_pt Argo 回源入口端口（本地），默认值8001
不建议修改,因为cf 隧道里的HTTP也是这个端口，不传就默认8001即可。

❗注意：nginx_pt与argo_pt的值不能同时为8001，不然会导致监听混乱(换句人话：nginx_pt不能设置为8001)。
...
## 解释一下上面那一大堆参数：
 ...
 0、 如果bash后面跟了一个参数 rep（如果没写这个参数默认视为安装），代表覆盖式安装（会卸载后再安装），你可以用这个改成其他功能，比如del 代表 卸载, list 代表 查看节点，具体有哪些值你可以跑一次安装脚本你就知道怎么用了。
...
7、cdn_host 指的是用argo时的cf域名，缺省值为cdn.7zz.cn，你可以自己传你要的值，比如 www.visa.com 。cdn_pt是cf域名对应的端口。 不传就会使用缺省值做兜底。
...
11、vl_sni_pt 指的是vless协议的sni(伪装域名)对应的握手端口，默认是443，你可在安装时自定义为https系那几个中的一个。
