...
# 默认节点名称常量
# 根据文件名自动判断协议类型
if [[ "${0##*/}" == *"vless"* || "${0##*/}" == *"sing-box"* ]]; then
    DEFAULT_NODE_NAME="$AUTHOR-vless"
else
...
EOF
    # 如果提供了ARGO_DOMAIN和ARGO_AUTH环境变量，则自动配置固定隧道
    if [ -n "$ARGO_DOMAIN" ] && [ -n "$ARGO_AUTH" ]; then
        yellow "检测到环境变量 ARGO_DOMAIN 和 ARGO_AUTH，正在自动配置固定隧道..."
...
vless://${uuid}@${server_ip}:${vless_port}?encryption=none&flow=xtls-rprx-vision&security=reality&sni=${sni_value}&fp=firefox&pbk=${public_key}&type=tcp&headerType=none#${node_name}
EOF
echo ""
...
check_nodes() {
    ...
    # 显示二维码
    green "\nSurge订阅二维码:"
    $work_dir/qrencode "https://sublink.eooce.com/surge?config=${base64_url}"
    
    green "\nsing-box订阅二维码:"
    $work_dir/qrencode "https://sublink.eooce.com/singbox?config=${base64_url}"
    
    green "\nMihomo/Clash系列订阅二维码:"
    $work_dir/qrencode "https://sublink.eooce.com/clash?config=${base64_url}"
...
is_interactive_mode() {
    # 检查是否有环境变量参数
    if [ -n "$PORT" ] || [ -n "$UUID" ] || [ -n "$SNI" ] || [ -n "$ARGO_DOMAIN" ] || [ -n "$ARGO_AUTH" ]; then
        return 1  # 非交互式模式
    else
        return 0  # 交互式模式
    fi
